...
class Employeex {
    ...
    public double getSalarym(int month){
        System.out.println(this.getName() + x + "月份工资为" + this.getSalary(x));

        //salary 属性需要在各subclass自己定义吗？因为salary需要private
        //return 0 和不写有什么区别，salary是否需要按月分，如输入int month
    }
...
class SalariedEmployeex extends Employeex {
    ...
    public double getSalary(int month){
        if(month==getBirthMonth()){
            ...
            x= monthlySalary;
        public double setSalary(x);
        public double getSalarym(month);
    }
}
